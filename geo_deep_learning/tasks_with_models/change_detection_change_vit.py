...
class ChangeDetectionChangeVIT(LightningModule):
    ...
    def __init__(  # noqa: PLR0913
        ...
        self.backbone = backbone if backbone else 'vit_small_patch14_dinov2'
    ...
    def training_step(
        ...
        """Run training step."""
        image, y, one_hot, logits, loss, main_loss, ce_loss, batch_size = self._forward_and_get_loss(batch)
        # --- Logging ---
        self.log(
            "train_loss",
            loss,
    ...
    def test_step(
        ...
        if self._total_samples_visualized < self.max_samples:
            ...
            samples_to_visualize = min(remaining, len(image))
    ...
    def _forward_and_get_loss(self, batch: dict[str, Any]) -> tuple[
      Any, Any, Tensor, Any, float | Any, Any, Any, Any
        ...
        common_data_mask = torch.nan_to_num(common_data_mask, nan=0.0, posinf=0.0, neginf=0.0)
        with torch.no_grad():
            print(
            "[DEBUG] x_pre stats:",
            "min", image.min().item(),
        ...
        if valid_sum == 0:
            ...
            loss = main_loss
            return image, y_float, one_hot, logits, loss, main_loss, ce_loss, batch_size
        ...
        return image, y_float, one_hot, logits, main_loss, loss, ce_loss, batch_size
