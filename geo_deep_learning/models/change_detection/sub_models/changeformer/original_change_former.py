...
class DecoderTransformer_v4(nn.Module):
    """Transformer Decoder v4 — uses SmoothUpsampleConv instead of ConvTranspose2d."""

    def __init__(self, input_transform='multiple_select', in_index=[0, 1, 2, 3], align_corners=False,
                 in_channels=[64, 128, 320, 512], embedding_dim=256, output_nc=2,
                 decoder_softmax=False, feature_strides=[4, 8, 16, 32], dropout_rate=0.1):
        super().__init__()
        assert len(feature_strides) == len(in_channels)
        self.feature_strides = feature_strides
        self.input_transform = input_transform
        ...
        self.linear_fuse = nn.Sequential(
            nn.Conv2d(self.embedding_dim * len(in_channels), self.embedding_dim, kernel_size=1),
            nn.BatchNorm2d(self.embedding_dim),
        )
        self.dropout = nn.Dropout2d(p=dropout_rate)

        # Smooth upsample (no checkerboard)
        self.up2x_1 = SmoothUpsampleConv(self.embedding_dim, self.embedding_dim)
        self.dense_2x = nn.Sequential(ResidualBlock(self.embedding_dim))
        self.up2x_2 = SmoothUpsampleConv(self.embedding_dim, self.embedding_dim)
        self.dense_1x = nn.Sequential(ResidualBlock(self.embedding_dim))
        self.change_probability = ConvLayer(self.embedding_dim, self.output_nc, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, inputs1, inputs2):
        ...
        _c4_2 = self.linear_c4(c4_2).permute(0, 2, 1).reshape(n, -1, c4_2.shape[2], c4_2.shape[3])
        _c4 = self.diff_c4(torch.cat((_c4_1, _c4_2), dim=1))
        outputs.append(self.make_pred_c4(_c4))
        _c4_up = resize(_c4, size=c1_2.size()[2:], mode='bilinear', align_corners=False)

        _c3_1 = self.linear_c3(c3_1).permute(0, 2, 1).reshape(n, -1, c3_1.shape[2], c3_1.shape[3])
        _c3_2 = self.linear_c3(c3_2).permute(0, 2, 1).reshape(n, -1, c3_2.shape[2], c3_2.shape[3])
        _c3 = self.diff_c3(torch.cat((_c3_1, _c3_2), dim=1)) + F.interpolate(_c4, size=c3_1.shape[2:], mode="bilinear", align_corners=False)
        outputs.append(self.make_pred_c3(_c3))
        _c3_up = resize(_c3, size=c1_2.size()[2:], mode='bilinear', align_corners=False)

        _c2_1 = self.linear_c2(c2_1).permute(0, 2, 1).reshape(n, -1, c2_1.shape[2], c2_1.shape[3])
        _c2_2 = self.linear_c2(c2_2).permute(0, 2, 1).reshape(n, -1, c2_2.shape[2], c2_2.shape[3])
        _c2 = self.diff_c2(torch.cat((_c2_1, _c2_2), dim=1)) + F.interpolate(_c3, size=c2_1.shape[2:], mode="bilinear", align_corners=False)
        outputs.append(self.make_pred_c2(_c2))
        _c2_up = resize(_c2, size=c1_2.size()[2:], mode='bilinear', align_corners=False)

        _c1_1 = self.linear_c1(c1_1).permute(0, 2, 1).reshape(n, -1, c1_1.shape[2], c1_1.shape[3])
        _c1_2 = self.linear_c1(c1_2).permute(0, 2, 1).reshape(n, -1, c1_2.shape[2], c1_2.shape[3])
        _c1 = self.diff_c1(torch.cat((_c1_1, _c1_2), dim=1)) + F.interpolate(_c2, size=c1_1.shape[2:], mode="bilinear", align_corners=False)
        outputs.append(self.make_pred_c1(_c1))
        ...
        if self.apply_output_activation:
            outputs = [self.active(p) for p in outputs]
        return outputs


class ChangeFormerV7(nn.Module):
    """ChangeFormer V7 — same encoder as V6, decoder without checkerboard artifacts.

    Uses DecoderTransformer_v4 (SmoothUpsampleConv) instead of DecoderTransformer_v3 (ConvTranspose2d).
    """
    ...
    def __init__(self, input_nc=3, output_nc=2, decoder_softmax=False, embed_dim=256):
        ...
        self.Tenc_x2 = EncoderTransformer_v3(
            img_size=256, patch_size=3, in_chans=input_nc, num_classes=output_nc,
            embed_dims=self.embed_dims, num_heads=[1, 2, 4, 8], mlp_ratios=[4, 4, 4, 4],
            qkv_bias=True, qk_scale=None, drop_rate=self.drop_rate,
            attn_drop_rate=self.attn_drop, drop_path_rate=self.drop_path_rate,
            norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=self.depths, sr_ratios=[8, 4, 2, 1],
        )

        self.TDec_x2 = DecoderTransformer_v4(
